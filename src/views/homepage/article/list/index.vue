      ...
      <el-table
        ...
        <el-table-column
          ...
          <template slot-scope="scope">
            ...
            <el-button v-has="'/system/pc/website/nav/add'" type="primary" plain size="mini" v-if="scope.row.parentId == 0" @click="handleAddSubclass(scope.row.id)">添加</el-button>
            <el-button v-has="'/system/pc/website/nav/article/view'" type="primary" plain size="mini" v-if="scope.row.parentId !== 0" @click="handleArticala(scope.row.id)">文章管理</el-button>
          </template>
        </el-table-column>
      </el-table>
      <div class="mgt20">
...
export default {
  ...
  methods: {
    handleArticala(id) {
      this.$router.push({ path: '/homepage/article/info', query: { navId: id }});
    },
    //新增
