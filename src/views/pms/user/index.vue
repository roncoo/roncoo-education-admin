  ...
  <div class="pad20">
    ...
    <div>
      <el-table v-loading="ctrl.load" size="medium" :data="list" stripe border style="width: 100%">
        ...
        >
          <template slot-scope="scope">
            <el-button v-has="'/system/pc/sys/role/list'" type="warning" plain size="mini" @click="handleUserRole(scope.row.id, scope.row.realName)">角色</el-button>
            <el-button v-has="'/system/pc/sys/user/password'" type="success" plain size="mini" @click="handlePassword(scope.row.userNo, scope.row.realName)">密码</el-button>
            <el-button v-has="'/system/pc/sys/user/view'" type="success" plain size="mini" @click="handleEdit(scope.row.id)">修改</el-button>
    ...
    />
    <add :visible="ctrl.addDialogVisible" :title="ctrl.dialogTitle" @close-callback="closeCallback" />
    <edit :visible="ctrl.dialogVisible" :form-data="formData" :title="ctrl.dialogTitle" @close-callback="closeCallback" />
    <password :visible="ctrl.passwordDialogVisible" :form-data="formData" :title="ctrl.dialogTitle" @close-callback="closeCallback" />
    <role :visible="ctrl.serRoleDialogVisible" :user-id="id" :title="ctrl.dialogTitle" @close-callback="closeCallback" />
  </div>
</template>
<script>
  ...
  methods: {
    ...
    handleEdit(id) {
      ...
      api.userView({id: id}).then(res => {
        ...
        this.ctrl.dialogTitle = res.data.realName
    ...
    handlePassword(userNo, realName) {
      ...
      this.ctrl.dialogTitle = realName
    },
    // 跳出设置角色弹窗
    ...
    adminUserList() {
      this.ctrl.load = true
      api.userPage(this.map, this.page.pageCurrent, this.page.pageSize).then(res => {
