  ...
  <div class="pad20">
    <div>
      <el-form :inline="true" size="mini">
        <el-form-item label="名称：">
          <el-input v-model.trim="map.roleName" />
        </el-form-item>
    ...
    <div>
      <el-table v-loading="ctrl.load" size="medium" :data="list" stripe border style="width: 100%">
        ...
        >
          <template slot-scope="scope">
            <el-button v-has="'/system/pc/menu/list'" type="warning" plain size="mini" @click="handlePms(scope.row.id, scope.row.roleName)">设置权限</el-button>
            <el-button v-has="'/system/pc/sys/role/view'" type="success" plain size="mini" @click="handleEdit(scope.row.id)">修改</el-button>
    ...
    <div>
      ...
      <edit :visible="ctrl.editDialogVisible" :form-data="formData" :title="ctrl.dialogTitle" @close-callback="closeCallback" />
      <pms :id="id" :visible="ctrl.pmsDialogVisible" :title="ctrl.dialogTitle" @close-callback="closeCallback" />
    </div>
  </div>
</template>
<script>
...
export default {
  ...
  methods: {
    ...
    roleList() {
      this.ctrl.load = true
      api.rolePage(this.map, this.page.pageCurrent, this.page.pageSize).then(res => {
    ...
    handleEdit(id) {
      ...
      api.roleView({id: id}).then(res => {
        ...
        this.ctrl.dialogTitle = res.data.roleName
      }).catch(() => {
        this.ctrl.load = false
      })
    },
    handlePms(id, roleName) {
      ...
      this.ctrl.dialogTitle = roleName
    },
    // 关闭弹窗回调
    closeCallback() {
