  ...
  <div class="pad20">
    <div>
      <el-form :inline="true" size="mini">
        <el-form-item label="菜单名称：">
          <el-input v-model.trim="map.menuName" />
        </el-form-item>
        <el-form-item>
    ...
    <div>
      ...
      >
        ...
        >
          <template slot-scope="scope">
            <el-button v-has="'/system/pc/menu/view'" type="success" plain size="mini" @click="handleEdit(scope.row.id)">编辑</el-button>
            <el-button v-has="'/system/pc/menu/add'" type="primary" plain size="mini" @click="addSubMmenu(scope.row.id)">添加</el-button>
            <el-button type="danger" plain size="mini" @click="handleDelete(scope.row.id)">删除</el-button>
          </template>
        </el-table-column>
      </el-table>
    </div>
    <add :visible="ctrl.addDialogVisible" :parent-id="parentId" :title="ctrl.dialogTitle" @close-callback="closeCallback" />
  ...
  methods: {
    ...
    handleEdit(id) {
      ...
      api.menuView({id: id}).then(res => {
        ...
        this.ctrl.dialogTitle = res.data.menuName
