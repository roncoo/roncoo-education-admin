<template>
  <el-dialog v-model="visible" :append-to-body="true" :title="formModel.data.id ? '修改' : '添加'" :width="600" center @close="cloneDialog">
    <el-form ref="ruleForm" :model="formModel.data" :rules="formModel.rules" class="demo-ruleForm" label-width="80px" @submit.prevent>
      <el-form-item label="导航名称" prop="articleTitle">
        <el-input v-model="formModel.data.articleTitle" maxlength="255" show-word-limit></el-input>
      </el-form-item>
      <el-form-item label="导航地址" prop="articleUrl">
        <el-input v-model="formModel.data.articleUrl" maxlength="255" show-word-limit></el-input>
      </el-form-item>
      <el-form-item label="跳转方式" prop="target">
        <el-select v-model="formModel.data.target" filterable placeholder="请选择">
...
import {articleEdit, articleSave} from '@/api/system.js';
...
export default defineComponent({
  ...
  setup(props, {emit}) {
    ...
    let formModel = reactive({
      ...
      rules: {
        articleTitle: [{required: true, message: '不能为空', trigger: 'blur'}],
        articleUrl: [{required: true, message: '不能为空', trigger: 'blur'}],
    ...
    const onSubmit = () => {
      ...
      ruleForm['value'].validate(async(valid) => {
        if (valid) {
          ...
          if (data.id) {
            d = await articleEdit(data);
          } else {
            d = await articleSave(data);
          }
          if (d) {
