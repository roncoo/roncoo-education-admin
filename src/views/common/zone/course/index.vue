<template>
  <div class="app-container">
    ...
    <el-table
      ...
      :data="page.list"
      :tree-props="{ children: 'periodViewRespList' }"
      border
      default-expand-all
      row-key="id"
    >
      ...
      <el-table-column :width="100" label="封面">
        <template #default="scope">
          <img
            :alt="scope.row.courseViewResp.courseName"
            :src="scope.row.courseViewResp.courseLogo"
          />
        </template>
      </el-table-column>
      ...
      <el-table-column :width="100" label="价格">
        <template #default="scope">
          ...
          <span v-if="scope.row.courseViewResp.isFree == 0">
            ￥{{ scope.row.courseViewResp.coursePrice }} <br /><span
              style="text-decoration: line-through"
              >￥{{ scope.row.courseViewResp.rulingPrice }}</span
            >
          </span>
        </template>
      </el-table-column>
      <el-table-column :width="100" label="售卖">
        <template #default="scope">
          <span :class="{ 'c-special': scope.row.courseViewResp.isPutaway === 0 }">{{
            getEnumObj('PutawayEnum')[scope.row.courseViewResp.isPutaway]
          }}</span>
        </template>
      </el-table-column>
