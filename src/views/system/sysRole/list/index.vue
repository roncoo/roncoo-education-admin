<template>
  <div class="app-container">
    <el-form class="filter-container" inline label-width="100px">
      ...
      <el-form-item>
        <el-button v-if="checkPermission('base:boss:sys:role:page')" type="primary" @click="listForQuery">查询</el-button>
        <el-button v-if="checkPermission('base:boss:sys:role:page')" @click="handleReset">刷新</el-button>
        <el-button v-if="checkPermission('base:boss:sys:role:save')" plain type="success" @click="handleAddRow">新增
        </el-button>
      </el-form-item>
    </el-form>
    ...
    >
      ...
      <el-table-column label="操作" width="360">
        <template #default="scope">
          <el-button v-if="checkPermission('base:boss:sys:role:allocation:menu')" plain type="warning"
                     @click="handleSettRow(scope.row)">菜单权限
          </el-button>
          <el-button v-if="checkPermission('base:boss:sys:role:edit')" plain type="primary"
                     @click="handleUpdateRow(scope.row.id)">编辑
          </el-button>
          <el-button v-if="checkPermission('base:boss:sys:role:update:status') && scope.row.status ===1 " plain
                     type="danger" @click="handleUpdateStatus(scope.row)">禁用
          </el-button>
          <el-button v-if="checkPermission('base:boss:sys:role:update:status') && scope.row.status ===0 " plain
                     type="success" @click="handleUpdateStatus(scope.row)">启用
          </el-button>
          <el-button v-if="checkPermission('base:boss:sys:role:delete')" plain type="danger"
    ...
    <edit-sys-role v-if="editCtr.visible" :info="editCtr.info" :title="editCtr.title" :visible="editCtr.visible"
                   @closes="closeEdit"/>
    <sett-sys-role v-if="settCtr.visible" :info="settCtr.info" :title="settCtr.title" :visible="settCtr.visible"
                   @closes="closeSett"/>
  </div>
</template>
